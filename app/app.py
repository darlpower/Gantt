...
if not survey_df.empty:
    ...
    survey_summary = pd.DataFrame(columns=["步道", "調查開始", "調查結束"])
...
if not geology_df.empty:
    ...
    geology_summary = pd.DataFrame(columns=["步道", "調查開始", "調查結束"])
...
if not qslope_df.empty:
    ...
    qslope_summary = pd.DataFrame(columns=["步道", "調查開始", "調查結束"])

# 篩選出「高精度UAV測繪」及「三維模型建模」的資料
# 1. 篩選 Task 中包含「高精度UAV測繪」或「三維模型建模」的資料
...
# 3. 依開始時間排序，利用 na_position="last" 把空值排到最後
...
app.layout = html.Div([
    ...
    html.H2("各步道高精度UAV測繪與三維模型建模時間摘要", style={'fontSize': '28px'}),
    dash_table.DataTable(
        id="uav-model-table",
        columns=[{"name": col, "id": col} for col in ["Team", "步道", "Task", "Start", "Finish"]],
        data=uav_model_df[["Team", "步道", "Task", "Start", "Finish"]].to_dict('records'),
        style_table={'overflowX': 'auto'},
        page_size=50,
        style_cell={'textAlign': 'center', 'fontSize': '21px'}
    )
